# Hi, I'm Angel 👋

**Machine Learning Engineer | Medical Imaging & Computer Vision**

I build deep learning systems that solve real clinical problems, from diabetic retinopathy detection to brain tumour segmentation.

---

### 🔬 What I Work On

- **Medical Image Analysis** — retinal scans, MRI, CT imaging
- **Deep Learning** — EfficientNet, ResNet, U-Net architectures
- **Interpretable AI** — Grad-CAM visualizations for clinical explainability
- **End-to-End Systems** — model training to deployed web applications

---

### 🚀 Featured Projects

| Project | Description | Stack |
|---|---|---|
| [RetinaLens AI](https://github.com/egwaojeangel/retinalens-ai-diabetic-retinopathy-detection) | Diabetic retinopathy detection across 5 severity grades with Grad-CAM | PyTorch, EfficientNet-B4, JavaScript |
| [Brain Tumour Segmentation](https://github.com/egwaojeangel/deep_learning_based_brain_tumor_segmentation_using_unet) | Automated MRI tumour segmentation using U-Net | PyTorch, U-Net |
| [Lung Cancer Detection](https://github.com/egwaojeangel/lung_cancer_detection_using_resnet50_on_CT_scans) | CT scan classification using ResNet50 | PyTorch, ResNet50 |

---

### 🛠️ Tech Stack

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![HTML](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![TailwindCSS](https://img.shields.io/badge/Tailwind-06B6D4?style=flat&logo=tailwindcss&logoColor=white)

---

### 📫 Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/angel-egwaoje-416927280)
[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=flat&logo=github&logoColor=white)](https://egwaojeangel.github.io)

---

*Currently working on a new project named "SkillBridge". Updates coming soon.* 👀
